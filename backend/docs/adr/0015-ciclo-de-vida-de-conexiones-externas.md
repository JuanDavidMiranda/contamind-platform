...
5. La sincronización de terceros se ejecuta por una página y guarda un cursor opaco en la fuente. Los reintentos de red/transitorios se aplican en `ProviderHttpClient`; un fallo conserva el cursor anterior, deja una corrida `failed` y desactiva operativamente la fuente hasta una nueva prueba de conexión.
...
- `POST /api/v1/data-sources/{id}/sync/parties` — procesa una página con cursor (`owner`/`admin`/`operator`).
...
- Las primeras sincronizaciones son síncronas y de una página para tener consistencia transaccional y trazabilidad. Un worker/cola podrá consumir las mismas corridas y cursores para volúmenes mayores sin cambiar el contrato público.
