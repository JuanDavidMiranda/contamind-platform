...
Esta versión no valida en línea los estados, no transmite facturas ni sustituye el proceso completo de facturación electrónica DIAN. Los campos `electronic_status`, `electronic_reference` y `electronic_status_at` conservan evidencia proveniente de una fuente contable o electrónica autorizada; un estado importado no equivale a una validación en línea con DIAN. Existe únicamente el piloto controlado `GetAcquirer`, descrito más adelante. El envío y la consulta de estados continúan pendientes. Ver `backend/docs/adr/0024-agente-de-facturacion-electronica.md`.
...
### Piloto DIAN: completar adquirientes para facturación electrónica

El primer vertical DIAN implementado es la consulta individual **GetAcquirer**. Está destinada exclusivamente a completar nombre o razón social y correo del adquiriente mientras se expide una factura electrónica o documento equivalente electrónico; no es una consulta de RUT, no infiere responsabilidades fiscales y no admite búsquedas por lote.

El cliente construye el mensaje SOAP 1.2 de la guía DIAN: Basic Auth, `wsa:Action`, `wsa:To` firmado, `WS-Security Signature`, `BinarySecurityToken`, `Timestamp` de 60 segundos con milisegundos, certificado PKCS#12 y el encabezado `Content-Type` exigido. Cada empresa configura una fuente `fiscal_authority`/`fiscal_service` con `provider_id: dian`, y sus secretos se cifran por fuente y empresa. Los campos requeridos son `software_id`, `software_password`, `certificate_pfx_base64` y `certificate_password`; nunca deben copiarse a `.env`, código, logs, tickets ni archivos de ejemplo.

Para habilitar una prueba controlada se requiere: (1) la habilitación DIAN del software y el certificado vigente de pertenencia de empresa, (2) `DIAN_INTEGRATION_ENABLED=true`, (3) `DIAN_ACQUIRER_URL` con la URL HTTPS que DIAN muestra en `Participants > Facturador` para ese ambiente - se acepta la URL WSDL y se invoca el `.svc` - y (4) crear la fuente y cargar los secretos mediante la API autenticada. La consulta se realiza con `POST /api/v1/companies/{company_id}/dian/acquirers/lookup`, exige un rol `owner`, `admin` u `operator`, `purpose: "electronic_invoice_issuance"` y `confirmed: true`.

La auditoría conserva actor, empresa, fuente, tipo de documento, correlación, resultado y un HMAC del número consultado. No conserva el número, el nombre, el correo ni el payload SOAP. El piloto no debe activarse en producción ni presentarse como una integración de facturación electrónica completa. La DIAN mantiene la [guía de consumo de Web Services](https://www.dian.gov.co/impuestos/factura-electronica/Documents/Guia-Herramienta-para-el-Consumo-de-Web-Services.pdf), la [documentación técnica del sistema de facturación electrónica](https://micrositios.dian.gov.co/sistema-de-facturacion-electronica/documentacion-tecnica/) y las reglas de uso individual de GetAcquirer en la [Resolución 000202 de 2025](https://normograma.dian.gov.co/dian/compilacion/docs/resolucion_dian_0202_2025.htm).

El flujo independiente de **Habilitación DIAN para software propio** permite configurar por empresa sus datos fiscales, credenciales cifradas, TestSet, política de firma, rangos de prueba y ZIP UBL firmado, y sigue las respuestas oficiales sin habilitar producción. Su alcance, operación y límites están documentados en [Habilitación DIAN con software propio](docs/dian-habilitacion-software-propio.md). Las credenciales y el PFX se cargan solo por la interfaz autorizada; nunca por chat, `.env`, tickets o repositorio.
